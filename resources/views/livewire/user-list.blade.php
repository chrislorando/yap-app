<section class="mt-16 pb-28 h-[90vh] overflow-y-auto bg-slate-800 z-20">
  <div wire:poll.visible class="relative">
      @foreach($models as $row)
      <div wire:key='{{ $row->id }}' class="z-50 border-b border-gray-500 cursor-pointer {{ $isActiveChat==$row->id ? 'bg-slate-800' : '' }}" wire:click='chat({{ $row->id }}, "{{ $row->name }}")' >
          <div class="px-4 py-5 ">
